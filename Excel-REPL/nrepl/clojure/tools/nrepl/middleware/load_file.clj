...
(defn- load-large-file-code
  ...
  (let [t (Environment/TickCount)                                                              ;DM: System/currentTimeMillis
    ...
    (swap! file-contents
      (fn [file-contents]
        (let [expired-keys
              (filter
                (comp #(and %
                            (< 100000000 (- (Environment/TickCount) %)))                       ;DM: (< 10000 (- (System/currentTimeMillis) %))   -- need to switch to ticks
                      :t meta)
                (keys file-contents))]
    ...
    (binding [*print-length* nil
      ...
      (pr-str `(try
                 (clojure.lang.Compiler/load
                   (System.IO.StringReader. (@@(var file-contents) '~file-key))                 ;DM: java.io.StringReader.
                   ~file-path
                   ~file-name)
                 (finally
