...
(ns #^{:author "Meikel Brandmeyer, modified for ClojureCLR by David Miller"
  ...
  (:import (System.IO IOException EndOfStreamException             ;;; (java.io IOException EOFException ByteArrayOutputStream
                   Stream MemoryStream )                           ;;; InputStream OutputStream PushbackInputStream) 
           clojure.lang.RT  clojure.lang.PushbackInputStream))     ;;; Added PushbackInputStream
...
(defn #^{:private true :tag |System.Byte[]|} read-bytes                                               ;;; :tag "[B"
  ...
  (let [content (byte-array n)]
    (loop [offset (int 0)
           len    (int n)]
      (let [result (.Read input content offset len)]                                                  ;;; .read
        (when (and (pos? len) (zero? result))                                                         ;;; (when (neg? result)
          (throw
...
(defn #^{:private true :tag |System.Byte[]|} string>payload                              ;;; :tag "[B"
  [#^String s]
  (.GetBytes System.Text.Encoding/UTF8 s))                                                ;;; (.getBytes s "UTF-8"))
...
(defmethod write-bencode :list
  [#^Stream output lst]                                          ;;; #^OutputStream
  (.WriteByte output (byte l))                                   ;;; (.write output (int l))
  (doseq [elt lst]
    (write-bencode output elt))
  (.WriteByte output (byte e)))                                       ;;; (.write output (int e))
...
(defmethod write-bencode :map
  ...
  (let [translation (into {} (map (juxt string>payload identity) (keys m)))
    ...
    (.WriteByte output (byte d))                                                   ;;; (.write output (int d))
    (doseq [k key-strings]
...
(defn #^{:private true} lexicographically
  [#^|System.Byte[]| a #^|System.Byte[]| b]                                                        ;;; #^"[B"  #^"[B"
  (let [alen (alength a)
        blen (alength b)
        len  (min alen blen)]
    (loop [i 0]
